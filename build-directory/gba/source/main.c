	...
	while (1) {
		if(REG_HS_CTRL&JOY_READ)
		{
			...
			else if(choseval == 2)
			{
				...
				REG_JOYTR = savesize;
				iprintf("\x1b[2J");
				iprintf("\x1b[9;3HSave read, sending save.\n");
				//wait for a cmd receive for safety
				while((REG_HS_CTRL&JOY_WRITE) == 0) ;
				REG_HS_CTRL |= JOY_RW;
				//send the save
				...
				iprintf("\x1b[2J");
				iprintf("\x1b[9;9HSave sent.\n");
				iprintf("\x1b[10;5HPlease turn off GBA.\n");
				kill = true;
			}
			else if(choseval == 3)
			{
				REG_JOYTR = savesize;
				iprintf("\x1b[2J");
				iprintf("\x1b[9;7HReceiving save.\n");
				...
				//write it
				iprintf("\x1b[2J");
				iprintf("\x1b[9;1HSave received, writing save.\n");
				switch (savesize){
				case 0x200:
